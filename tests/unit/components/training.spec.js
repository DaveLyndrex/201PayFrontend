...
import training from "../../../src/views/human-resources/setups/Training.vue";
...
describe("Component", () => {
    ...
    it("input config", () => {
        expect(JSON.stringify(getMountedComponent(training, {}).vm.inputConfig)).toBe(
            JSON.stringify([{
    ...
    it("contains name and assignment", () => {
        expect(getMountedComponent(training).vm.headers).toEqual([
    ...
    it("should find and trigger button cancel", async() => {
        const wrapper = getMountedComponent(training, {});
...
describe("test training method", () => {
    ...
    beforeEach(() => {
        ...
        wrapper = shallowMount(training, { user_session, user_details, localVue });
    })
