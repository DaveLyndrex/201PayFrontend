...
import timekeeping from "../../../src/views/human-resources/setups/Timekeeping.vue";
...
describe("Component", () => {
    ...
    it("input config", () => {
        expect(JSON.stringify(getMountedComponent(timekeeping, {}).vm.inputConfig)).toBe(
            JSON.stringify([{
                inputLabel: "Timekeeping",
                inputType: "text",
                inputModel: "Timekeeping",
                rules: [(v) => Validation.required(v, "Timekeeping")],
            }, ])
        );
    });

    it("contains name and assignment", () => {
        expect(getMountedComponent(timekeeping).vm.headers).toEqual([
            { text: "Timekeeping", value: "Timekeeping" },
    ...
    it("should find and trigger button cancel", async() => {
        const wrapper = getMountedComponent(timekeeping, {});
...
describe("test timekeeping method", () => {
    ...
    beforeEach(() => {
        ...
        wrapper = shallowMount(timekeeping, { user_session, user_details, localVue });
    })
