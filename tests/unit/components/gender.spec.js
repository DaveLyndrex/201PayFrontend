...
import gender from "../../../src/views/human-resources/setups/Gender.vue";
...
describe("Gender.vue", () => {
    ...
    it("input config", () => {
        expect(JSON.stringify(getMountedComponent(gender, {}).vm.inputConfig)).toBe(
            JSON.stringify(
            [
                {
                    inputLabel: "Sex",
    ...
    it("contains name and assignment", () => {
        expect(getMountedComponent(gender).vm.headers).toEqual([
            { text: "Sex", value: "Sex" },
    ...
    it("should find and trigger button cancel", async() => {
        const wrapper = getMountedComponent(gender, {});
...
describe("test gender method", () => {
    ...
    beforeEach(() => {
        ...
        wrapper = shallowMount(gender, { user_session, user_details, localVue });
    })

    test("retrieve data spyOn()", async() => {
        const wrapper = shallowMount(gender, {});
