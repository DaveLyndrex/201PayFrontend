...
import customFields from "../../../src/views/human-resources/setups/EmployeeCustomFields.vue";
...
describe("EmployeeCustomFields.vue", () => {
    beforeAll(() => {
    ...
    it("input config", () => {
        expect(JSON.stringify(getMountedComponent(customFields, {}).vm.inputConfig)).toBe(
            JSON.stringify(
            [
                {
    ...
    it("contains name and assignment", () => {
        expect(getMountedComponent(customFields).vm.headers).toEqual([
    ...
    it("should find and trigger button cancel", async() => {
        const wrapper = getMountedComponent(customFields, {});
...
describe("test customFields method", () => {
    ...
    beforeEach(() => {
        ...
        wrapper = shallowMount(customFields, { user_session, user_details, localVue });
    })
