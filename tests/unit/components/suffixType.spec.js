...
import suffixType from "../../../src/views/human-resources/setups/SuffixType.vue";
...
describe("Component", () => {
    ...
    it("input config", () => {
        expect(JSON.stringify(getMountedComponent(suffixType, {}).vm.inputConfig)).toBe(
            JSON.stringify([{
                inputLabel: "Suffix",
                inputType: "text",
                inputModel: "Suffix",
                rules: [(v) => Validation.required(v, "Suffix")],
            }, ])
        );
    });

    it("contains name and assignment", () => {
        expect(getMountedComponent(suffixType).vm.headers).toEqual([
    ...
    it("should find and trigger button cancel", async() => {
        const wrapper = getMountedComponent(suffixType, {});
...
describe("test suffixType method", () => {
    ...
    beforeEach(() => {
        ...
        wrapper = shallowMount(suffixType, { user_session, user_details, localVue });
    })
