...
import regularization from "../../../src/views/human-resources/setups/Regularization.vue";
...
describe("Component", () => {
    ...
    it("input config", () => {
        expect(JSON.stringify(getMountedComponent(regularization, {}).vm.inputConfig)).toBe(
            JSON.stringify([{
...

                {
                    ...
                    inputType: "number",
                    inputModel: "Default",
                    rules: [(v) => Validation.required(v, "Default")],
                },
            ])
        );
    });
    ...
    it("contains name and assignment", () => {
        expect(getMountedComponent(regularization).vm.headers).toEqual([
    ...
    it("should find and trigger button cancel", async() => {
        const wrapper = getMountedComponent(regularization, {});
...
describe("test regularization method", () => {
    ...
    beforeEach(() => {
        ...
        wrapper = shallowMount(regularization, { user_session, user_details, localVue });
    })
