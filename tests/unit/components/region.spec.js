...
import region from "../../../src/views/human-resources/setups/Region.vue";
...
describe("Region.vue", () => {
    ...
    it("input config", () => {
        expect(JSON.stringify(getMountedComponent(region, {}).vm.inputConfig)).toBe(
            JSON.stringify(
            [
                {
                    inputLabel: "Location",
    ...
    it("contains name and assignment", () => {
        expect(getMountedComponent(region).vm.headers).toEqual([
    ...
    it("should find and trigger button cancel", async() => {
        const wrapper = getMountedComponent(region, {});
...
describe("test region method", () => {
    ...
    beforeEach(() => {
        ...
        wrapper = shallowMount(region, { user_session, user_details, localVue });
    })

    test("retrieve data spyOn()", async() => {
        const wrapper = shallowMount(region, {});
