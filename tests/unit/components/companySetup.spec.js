...
import companySetup from "../../../src/views/human-resources/setups/CompanySetup.vue";
...
describe("CompanySetup.vue", () => {
    ...
    it("input config", () => {
        expect(JSON.stringify(getMountedComponent(companySetup, {}).vm.inputConfig)).toBe(
            JSON.stringify(
            [
                {
    ...
    it("contains name and assignment", () => {
        expect(getMountedComponent(bloodType).vm.headers).toEqual([
    ...
    it("should find and trigger button cancel", async() => {
        const wrapper = getMountedComponent(bloodType, {});
...
describe("test bloodType method", () => {
    ...
    beforeEach(() => {
        ...
        wrapper = shallowMount(bloodType, { user_session, user_details, localVue });
    })

    test("retrieve data spyOn()", async() => {
        const wrapper = shallowMount(bloodType, {});
