...
import workerType from "../../../src/views/human-resources/setups/WorkerType.vue";
...
describe("Component", () => {
    ...
    it("input config", () => {
        expect(JSON.stringify(getMountedComponent(workerType, {}).vm.inputConfig)).toBe(
            JSON.stringify([{
                inputLabel: "Worker Type",
                inputType: "text",
                inputModel: "WorkerType",
                rules: [(v) => Validation.required(v, "Worker Type")],
            }, ])
        );
    });

    it("contains name and assignment", () => {
        expect(getMountedComponent(workerType).vm.headers).toEqual([
            { text: "Worker Type", value: "WorkerType" },
    ...
    it("should find and trigger button cancel", async() => {
        const wrapper = getMountedComponent(workerType, {});
...
describe("test workerType method", () => {
    ...
    beforeEach(() => {
        ...
        wrapper = shallowMount(workerType, { user_session, user_details, localVue });
    })
