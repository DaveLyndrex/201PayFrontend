...
import prefix from "../../../src/views/human-resources/setups/Prefix.vue";
...
describe("Prefix.vue", () => {
    ...
    it("input config", () => {
        expect(JSON.stringify(getMountedComponent(prefix, {}).vm.inputConfig)).toBe(
            JSON.stringify(
            [
                {
                    inputLabel: "Prefix",
    ...
    it("contains name and assignment", () => {
        expect(getMountedComponent(prefix).vm.headers).toEqual([
    ...
    it("should find and trigger button cancel", async() => {
        const wrapper = getMountedComponent(prefix, {});
...
describe("test prefix method", () => {
    ...
    beforeEach(() => {
        ...
        wrapper = shallowMount(prefix, { user_session, user_details, localVue });
    })

    test("retrieve data spyOn()", async() => {
        const wrapper = shallowMount(prefix, {});
