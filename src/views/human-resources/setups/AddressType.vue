  ...
  <v-app>
    ...
    >
       <template v-slot:actionBtns>
          <v-btn class="mb-2 px-8 font-color cancel" color="whitesmoke" @click="close">
            Cancel
          </v-btn>
          <v-btn
...
export default {
  ...
  created() {
    ...
    EventBus.$on("saveItem", (props) => {
      if (this.editedIndex > -1) {
        Object.assign(this.addressType[this.editedIndex], props.item);
      } else {
        this.addressType.push(props.item);
      }
    });
  ...
  methods: {
    saveItem(data) {
      if (this.$refs.addressTypeForm.validateForm()) {
        if (this.editedIndex > -1) {
    ...
    retrieveData: async function () {
      ...
      try {
        let getdata = await this.retrieve(
          ...
          "setAddressTypes",
          "allAddressTypesGetters"
        );
        this.addressType = getdata ? getdata : [];
      } catch (error) {
        console.log(error);
      }
      this.loading = false;
...
//CREATE Address Type
    createNewItem: async function (data) {
      ...
      let create = await this.create(data, "createAddressType");
      // this.addressType = this.$store.getters.allAddressTypes;
      if (create) {
...
//DELETE Address Type
    deleteItem: async function (data) {
      this.savingChanges = true;
      console.log(data)
      let del = await this.delete(data, "deleteAddressType");
      if (del) {
        await this.retrieveData();
