        ...
        >
          <template v-slot:activator="{ on, attrs }">
            <v-text-field
              solo
              dense
              v-model="editedItem.EffectiveEndDateCompany"
              persistent-hint
              append-icon="mdi-calendar"
              v-bind="attrs"
      ...
      inputConfig: [
        ...
        },
       {
        ...
        },
        {
          inputLabel: "Set Code",
          inputType: "text",
          inputModel: "SetCodeCompany",
           rules: [(v) => Validation.required(v, "SetCodeCompany")],
        },
      ],
      editedItem: {
        ID: null,
  ...
  watch: {
    openForm(val) {
      ...
      if(this.edit === true){
          ...
          this.editedItem.SetCodeCompany = this.editedItem.SetCode
      }else{
          this.editedItem.EffectiveStartDate = null
          this.editedItem.EffectiveEndDateCompany = null
          this.editedItem.ClassificationEffectiveDate = null
          this.editedItem.ExtraInfoEffectiveStartDate = null
      }
    },
  ...
  methods: {
    saveItem(data) {
      if (this.$refs.companyForm.validateForm()) {
        ...
        console.log(parseData.SetCodeCompany)
        parseData.EffectiveStartDate = this.formatDateForBe(parseData.EffectiveStartDate);
        parseData.EffectiveEndDate = this.formatDateForBe(parseData.EffectiveEndDateCompany);
        parseData.ExtraInfoEffectiveStartDate = this.formatDateForBe(parseData.ExtraInfoEffectiveStartDate);
        parseData.ClassificationEffectiveDate = this.formatDateForBe(parseData.ClassificationEffectiveDate);
        parseData.SetCode = parseData.SetCodeCompany
        console.log(parseData.SetCode)
        if (this.editedIndex > -1) {
