...
export default {
  ...
  data() {
    return {
      ...
      message: "",
      headers: [
        { text: "Manager Type", value: "ManagerType" },
      ...
      inputConfig: [
        {
          inputLabel: "Manager Type",
          inputType: "text",
          inputModel: "ManagerType",
          rules: [(v) => Validation.required(v, "Manager Type")],
        },
      ],
      editedItem: {
        ID: null,
  ...
  created() {
    ...
    EventBus.$on("saveItem", (props) => {
      if (this.editedIndex > -1) {
        Object.assign(this.managerType[this.editedIndex], props.item);
      } else {
        this.managerType.push(props.item);
      }
    });
  ...
  methods: {
    saveItem(data) {
      if (this.$refs.managerTypeForm.validateForm()) {
        if (this.editedIndex > -1) {
    ...
    retrieveData: async function() {
      ...
      try {
        ...
        this.managerType = getdata ? getdata : [];
      } catch (error) {
        console.log(error);
      }
      this.loading = false;
    ...
    createNewItem: async function(data) {
      ...
      let create = await this.create(data, "createManagerType");
      this.managerType = this.$store.getters.allManagerType;
      if (create) {
    ...
    deleteItem: async function(data) {
      this.savingChanges = true;
      console.log(data);
      let del = await this.delete(data, "deleteManagerType");
      if (del) {
        await this.retrieveData();
      }
      this.savingChanges = false;
