  ...
  created() {
    ...
    EventBus.$on("saveItem", (props) => {
      if (this.editedIndex > -1) {
        Object.assign(this.ethnicity[this.editedIndex], props.item);
      } else {
        this.ethnicity.push(props.item);
      }
    });
  ...
  methods: {
    ...
    retrieveData: async function() {
      ...
      try {
        ...
        this.ethnicity = getdata ? getdata : [];
      } catch (error) {
        console.log(error);
      }
      this.loading = false;
    ...
    createNewItem: async function(data) {
      ...
      let create = await this.create(data, "addEthnicity");
      // this.ethnicity = this.$store.getters.allEthnicity;
      if (create) {
    ...
    deleteItem: async function(data) {
      this.savingChanges = true;
      try {
        let del = await this.delete(data, "deleteEthnicity");
