...
<!-- 10/13/2021 CN A.DIEZ -->
  <v-app>
    ...
    <template v-slot:StartDate>
      ...
      <template v-slot:MaritalStatusDate>
        <v-menu
          ref="maritalStatusDate"
          v-model="maritalStatusDate"
          :close-on-content-click="false"
          transition="scale-transition"
          offset-y
          max-width="290px"
        ...
        >
          <template v-slot:activator="{ on, attrs }">
            <v-text-field
              :rules="[v => validation.required(v, 'Marital Status Date')]"
              required
              solo
              dense
              v-model="editedItem.MaritalStatusDate"
              persistent-hint
              append-icon="mdi-calendar"
              v-bind="attrs"
              @blur="maritalSatusDateMenuValue = parseDate(editedItem.MaritalStatusDate)"
              v-on="on"
            ></v-text-field>
          </template>
          <v-date-picker
            v-model="maritalStatusDateMenuValue"
            no-title
            @input="maritalStatusDate = false"
          ></v-date-picker>
        </v-menu>
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Highest Educational Level", value: "HighestEducationalLevel" },
        { text: "Marital Status", value: "MaritalStatus" },
        { text: "Marital Status Date", value: "MaritalStatusDate" },
        { text: "Sex", value: "Sex" },
      ...
      inputConfig: [
        ...
        },
        {
          ...
          inputType: "text",
          inputModel: "MaritalStatus",
          rules: [
            v => Validation.required(v, 'Marital Status')
          ]
        },
        { inputLabel: "End Date", 
          inputType: "date", 
        ...
        {
          inputLabel: "Marital Status Date",
          inputType: "date",
          inputModel: "MaritalStatusDate",
          rules: [
            v => Validation.date(v, 'Marital Status Date')
          ]
        },
        {
          inputLabel: "Highest Educational Level",
          inputType: "text",
          inputModel: "HighestEducationalLevel",
          rules: [
            v => Validation.required(v, 'Highest Educational Level')
          ]
        },
       {
          inputLabel: "Sex",
          inputType: "select",
          inputModel: "Sex",
          selectionItems: this.$store.getters.allSexType.map((item) => {
            return {
              ...item,
              name: item.Sex,
      ...
      defaultItem: {
        ...
        HighestEducationalLevel: null,
        MaritalStatus: null,
        MaritalStatusDate: this.formatDate(this.getDefaultDate()),
        Sex: null,
        CreatedDate: null,
        ModifiedDate: null,
        ModifiedBy: JSON.parse(localStorage.getItem("user_details")).data.Email,
      ...
      maritalStatusDateMenuValue: this.getDefaultDate(),
      startDate: false,
      endDate: false,
      MaritalStatus: false,
      maritalStatusDate: false,
 ...
 watch: {
    ...
    maritalStatusDateMenuValue(val) {
      console.log(val);
      this.editedItem.MaritalStatusDate = this.formatDate(this.maritalStatusDateMenuValue);
    }
  },
  mounted() {
  ...
  created() {
     ...
     this.$store.watch(
      (state, getters) => getters.allSexType,
      (newValue) => {
        if (newValue) {
    ...
    EventBus.$on("saveItem", (props) => {
      if (this.editedIndex > -1) {
        Object.assign(this.legislativeData[this.editedIndex], props.item);
      } else {
        this.legislativeData.push(props.item);
      }
    });
  ...
  methods: {
    retrieveData: async function () {
      ...
      try {
        ...
        this.legislativeData = getData ? getData : [];
        this.loading = false;
      } catch (error) {
        console.log(error);
    ...
    saveItem(data) {
      if (this.$refs.legislativeForm.validateForm()) {
      data.Value = Number.parseFloat(String(data.Value).replace(/,/g, ""));
      const parseData = {...data} ;
      parseData.MaritalStatusDate = this.formatDateForBe(data.MaritalStatusDate);
    ...
    createNewItem: async function (data) {
      ...
      let create = await this.create(data, "createLegislativeData");
      this.legislativeData = this.$store.getters.allLegislative;
      if (create) {
        this.retrieveData();
        this.close();
    ...
    updateItem: async function (data) {
      ...
      let update = await this.update(
        data,
        this.editedIndex,
        "updateLegislativeData",
        "allLegislative"
      );

      if (update) {
        this.legislativeData = update;
        this.close();
        this.response = "success";
    ...
    deleteItem: async function (data) {
      this.savingChanges = true;
      let del = await this.delete(data, "deleteLegislative");
      if (del) {
        await this.retrieveData();
    ...
    initializeRelationalDatas: async function () {
      try {
        await this.retrieveSexType();
      } catch (error) {
        this.showWarningResponse("Oops!", "An error encountered!");
      }
    },

    retrieveSexType: async function () {
