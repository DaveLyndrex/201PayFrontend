...
export default {
    ...
    return {
      ...
      inputConfig: [
        ...
        },
        {
          ...
          inputType: "text",
          inputModel: "Responsibilities",
          rules: [(v) => Validation.required(v, "Responsibilities")],
        },
        {
          inputLabel: "End Date",
          inputType: "date",
        ...
        {
          inputLabel: "Reason For Leaving",
          inputType: "text",
          inputModel: "ReasonForLeaving",
          rules: [(v) => Validation.required(v, "Reason For Leaving")],
        },
        {
          inputLabel: "Company",
          inputType: "text",
        ...
        },
        {
          ...
          rules: [(v) => Validation.required(v, "Designation")],
        },
        {
  ...
  created() {
    ...
    EventBus.$on("saveItem", (props) => {
      if (this.editedIndex > -1) {
        Object.assign(this.jobHistoryData[this.editedIndex], props.item);
      } else {
        this.jobHistoryData.push(props.item);
      }
    });
  ...
  methods: {
    saveItem(data) {
      if (this.$refs.jobHistoryForm.validateForm()) {
    ...
    retrieveData: async function() {
      ...
      try {
        ...
        this.jobHistoryData = getdata ? getdata : [];
        this.loading = false;
      } catch (error) {
        console.log(error);
      }
    ...
    createNewItem: async function(data) {
      let create = await this.create(data, "createJobHistoryData");
      this.jobHistoryData = this.$store.getters.allJobHistoryData;
    ...
    deleteItem: async function(data) {
      this.savingChanges = true;
      let del = await this.delete(data, "deleteJobHistoryData");
      if (del) {
        await this.retrieveData();
        this.response = "success";
