...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Citizenship Status", value: "CitizenshipID" },
      ...
      inputConfig: [
        ...
        {
          inputLabel: "Citizenship Status",
          inputType: "select",
          inputModel: "CitizenshipID",
          selectionItems: this.$store.getters.allCitizenshipData.map((item) => {
            return {
              ...item,
              name: item.CitizenshipStatus,
            };
          }),
          rules: [(v) => Validation.required(v, "Citizenship Status")],
        },
        {
          inputType: "hidden",
  ...
  created() {
    this.$store.watch(
      (state, getters) => getters.allCitizenshipStatus,
      (newValue) => {
        if (newValue) {
    ...
    EventBus.$on("saveItem", (props) => {
      if (this.editedIndex > -1) {
        Object.assign(this.citizenshipData[this.editedIndex], props.item);
      } else {
        this.citizenshipData.push(props.item);
      }
    });
  ...
  methods: {
    saveItem(data) {
      if (this.$refs.citizenshipForm.validateForm()) {
    ...
    retrieveData: async function() {
      ...
      try {
        ...
        this.citizenshipData = getdata ? getdata : [];
    ...
    createNewItem: async function(data) {
      let create = await this.create(data, "createCitizenshipData");
      this.citizenshipData = this.$store.getters.allCitizenshipData;
    ...
    deleteItem: async function(data) {
      this.savingChanges = true;
      let del = await this.delete(data, "deleteCitizenship");
      if (del) {
        await this.retrieveData();
        this.response = "success";
    ...
    initializeRelationalDatas: async function() {
      try {
        await this.retrieveCitizenshipStatus();
      } catch (error) {
        this.showWarningResponse("Oops!", "An error encountered!");
      }
    },

    retrieveCitizenshipStatus: async function() {
